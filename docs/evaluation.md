...
- **answer_value_match**: exact-value containment (expected digits appear in reply).
- **abstention_correct**: abstain when gold says so; do not abstain when an answer is required.
- evidence/citation averages skip abstain-only cases.

## Gate 3 — frozen pilot numbers
...
Separates **EEFF chunk inject** from **IDP prompt rules** on the same n=10 gold ([`evals/rag_chat_v1.json`](../evals/rag_chat_v1.json)). Complementary to Gate 3; does not replace it.

**Live sequential ablation** over the same `demo_4` / `chat_demo_4` stack: arms run in order (off → eval → chunks → eval → full → eval → restore full). Chat state is shared across arms; this is not a fully isolated experimental design.
...
Chunk inject alone lifts value match; prompt rules close the rest (and abstention). Dump: `outputs/rag_ablation.json`.
...
- [`rag_retrieval_v1.json`](../evals/rag_retrieval_v1.json), [`rag_chat_v1.json`](../evals/rag_chat_v1.json)
